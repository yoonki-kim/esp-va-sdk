...
static const char* NEO_PIXEL_TAG = "Neo_pixel";
...
int leds_init(int cnt, int gpio, int no)
{
    ...
    if (cfg_ok != ESP_OK) {
        ESP_LOGE(NEO_PIXEL_TAG,"Can't configure RMT chan %d!\n", 0);
        return false;
    }
    esp_err_t install_ok = rmt_driver_install(0, 0, 0);
    if (install_ok != ESP_OK) {
        ESP_LOGE(NEO_PIXEL_TAG,"Can't install RMT ist for chan %d!\n", 0);
        return false;
    }
    /*
    ...
    */
    ...
    if (rmtdata == NULL) {
        ESP_LOGE(NEO_PIXEL_TAG,"Can't allocate data for %d leds on channel %d\n", ledcnt, 0);
        return false;
    }
    mux = xSemaphoreCreateMutex();
    if (mux == NULL) {
        ESP_LOGE(NEO_PIXEL_TAG,"Can't create semaphore\n");
        return false;
    }
    return true;
}
...
void init_led_colour()
{
    ...
    if (led_val == NULL) {
        ESP_LOGE(NEO_PIXEL_TAG,"memory allocation failed for calloc\n");
    }
}
...
esp_err_t neo_pixel_led_init(int led_cnt, int gpio_no)
{
    ...
    if (!res) {
        ESP_LOGE(NEO_PIXEL_TAG,"neo led init failed");
        return res;
    }
    init_led_colour();
    return res;
...
void va_led_set_pwm(const uint32_t *va_led_v)
{
    for (int i = 0; i < led_count; i++) {
        led_val[3 * i] = (va_led_v[i] & 0xff0000UL) >> 16;
        led_val[(3 * i) + 1] = (va_led_v[i] & 0x00ff00UL) >> 8;
        led_val[(3 * i) + 2] = (va_led_v[i] & 0x0000ffUL);
    }
    leds_send(led_val);
}